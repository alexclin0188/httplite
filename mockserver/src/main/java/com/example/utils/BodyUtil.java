...
public class BodyUtil {
    ...
    public final int DATAHEADER = 1;
    public final int FILEDATA = 2;
    public final int FIELDDATA = 3;

    public final int MXA_SEGSIZE = 1000 * 1024 * 10;//每批最大的数据量 10M

    private byte[] parse(RecordedRequest request) throws IOException {

        String contentType = request.getHeader("content-type");// 请求消息类型
        ...
        int state = NONE;
        // 得到数据输入流reqbuf
        DataInputStream in = new DataInputStream(request.getBody().inputStream());
        // 将请求消息的实体送到b变量中
        int totalBytes = (int) request.getBodySize();
        String message = "";
